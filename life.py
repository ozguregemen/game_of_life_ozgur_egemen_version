# This is a backup of the stable 2D Game of Life code before converting to 3D.
# You can copy this file back to life.py if you want to revert to the 2D version.

# ... copy entire contents of life.py here ...
...
def update_window_size(new_width, new_height):
    global WINDOW_WIDTH, WINDOW_HEIGHT, CELL_SIZE, WIDTH, HEIGHT, screen, main_menu, grid_offset_x, grid_offset_y
    global ROWS, COLS, grid, trail_grid, activity_grid
    ...
    # Calculate COLS and ROWS dynamically
    COLS = max(1, available_width // MIN_CELL_SIZE)
    ROWS = max(1, (available_height - GRID_TOP_MARGIN - GRID_BOTTOM_MARGIN) // MIN_CELL_SIZE)
    ...
    HEIGHT = ROWS * CELL_SIZE + GRID_TOP_MARGIN + GRID_BOTTOM_MARGIN
    grid_offset_x = 0
    grid_offset_y = INFO_BAR_HEIGHT
    # Recreate grid and related arrays
    grid = [[0 for _ in range(COLS)] for _ in range(ROWS)]
    trail_grid = [[0 for _ in range(COLS)] for _ in range(ROWS)]
    activity_grid = [[0 for _ in range(COLS)] for _ in range(ROWS)]
...
# Game state
grid = [[0 for _ in range(COLS)] for _ in range(ROWS)]  # 2D list grid
grid_history = []
current_rule = "conway"
...
grid_overlay.show_quadrants = False
...
# Add a trail grid to track recently dead cells
TRAIL_MAX = 10
trail_grid = [[0 for _ in range(COLS)] for _ in range(ROWS)]
activity_grid = [[0 for _ in range(COLS)] for _ in range(ROWS)]  # Track cell activity for heatmap
...
# Initialize window size and grid
...
def rastgele_grid(rows, cols, density=0.2):
...
def yerles_pattern(grid, pattern, x, y, rotation=0, flip_h=False, flip_v=False):
    ...
    for dx in range(len(pattern_data)):
        for dy in range(len(pattern_data[0])):
            if 0 <= x + dx < ROWS and 0 <= y + dy < COLS:
                grid[x + dx][y + dy] = pattern_data[dx][dy]
...
def clear_grid():
    """Clear the grid"""
    global grid, grid_history, nesil, activity_grid, trail_grid
    grid = [[0 for _ in range(COLS)] for _ in range(ROWS)]
    activity_grid = [[0 for _ in range(COLS)] for _ in range(ROWS)]
    trail_grid = [[0 for _ in range(COLS)] for _ in range(ROWS)]
    grid_history = []
...
def cycle_theme():
    global current_theme, main_menu, grid_overlay
...
def grid_ciz(grid, hiz, nesil, simulasyon_aktif, grid_goster=True, preview_pattern=None, preview_pos=None):
    global show_age_numbers, show_heatmap
    screen.fill(THEMES[current_theme]["background"])
    
    # Draw grid with smooth transitions
    for x in range(ROWS):
        for y in range(COLS):
            rect = pygame.Rect(
    ...
    for x in range(ROWS):
        for y in range(COLS):
            trail_age = trail_grid[x][y]
            if trail_age > 0 and grid[x][y] == 0:
    ...
    # Draw the rule overlay if needed
...
try:
    ...
    while True:
        ...
        for event in events:
            if main_menu.handle_event(event):
                ...
                if çizim_modu:
                    ...
                    gy = (mx - grid_offset_x) // CELL_SIZE
                    if 0 <= gx < ROWS and 0 <= gy < COLS:
                        old_value = grid[gx][gy]
                        grid[gx][gy] = çizim_tipi
        ...
        if simulasyon_aktif or adim_modu:
            # Only update if there are live cells
            if any(cell > 0 for row in grid for cell in row):
                # Update activity grid decay
                for x in range(ROWS):
                    for y in range(COLS):
                        activity_grid[x][y] = max(0, activity_grid[x][y] - 0.1)  # Decay activity over time
                        trail_grid[x][y] = max(0, trail_grid[x][y] - 1)  # Decay trail for all cells
                # Store grid history
                ...
                for x in range(ROWS):
                    for y in range(COLS):
                        if new_grid[x][y] != grid[x][y]:
                            cell_transition.start_transition(x, y, grid[x][y], new_grid[x][y])
                            activity_grid[x][y] += 1  # Increase activity for changed cells
                            if grid[x][y] > 0 and new_grid[x][y] == 0:
                                trail_grid[x][y] = TRAIL_MAX  # Set trail for newly dead cells
                grid = new_grid
                nesil += 1
                adim_modu = False
