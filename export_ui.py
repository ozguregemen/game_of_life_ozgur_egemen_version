...
class ExportMenuServices:
    ...
    export_slice_atlas: Callable[[], bool]
...
class ExportMenu:
    ...
    def entries(self) -> tuple[dict[str, object], ...]:
        ...
        is_three_d = context.startswith("3D")
        entries: list[dict[str, object]] = [
            {
                "key": "png",
                "name": "PNG 3D Viewport" if is_three_d else "PNG Diagram",
                "detail": (
                    "Full 1D space-time diagram"
                    if context.startswith("1D")
                    else "Current camera, clipping, colors, lighting, and opacity"
                    if is_three_d
            ...
            },
            {
                ...
                "name": "Animated GIF",
                "detail": (
                    "Up to 120 sampled OpenGL viewport frames"
                    if is_three_d
            ...
            },
            {
                ...
                "name": "MP4 Video",
                "detail": (
                    "H.264 camera-view timeline at 20 frames per second"
                    if is_three_d
        ...
        if is_three_d:
            entries.append(
                {
                    "key": "atlas",
                    "name": "PNG Orthogonal Slice Atlas",
                    "detail": "Deterministic XY / XZ / YZ scientific cross-sections",
                    "callback": self.services.export_slice_atlas,
                }
            )
        entries.extend(
            (
                {
                    ...
                    "detail": (
                        "Population, state/block entropy, Hamming and spatial metrics"
                    ),
    ...
    def geometry(
        ...
        preferred_height = 150 + len(entries) * 55
        modal = pygame.Rect(
            0,
            0,
            min(650, width - 40),
            min(preferred_height, height - 40),
        )
    ...
    def draw(self) -> None:
        ...
        footer = f"Mouse / 1-{len(cards)}: select  |  Esc or X: close"
        if self.runner.busy:
    ...
    def handle_event(self, event: pygame.event.Event) -> bool:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if 0 <= offset < len(entries):
                self.execute(entries[offset]["key"])
                return True
            return True
